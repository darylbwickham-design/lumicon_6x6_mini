...
void drawBar(int x, int y, int w, uint8_t value, uint16_t color);
...
void drawPetSprite() {
  uint8_t stateHash = (pet.stage << 4) ^ (pet.sleeping << 3) ^ (pet.sick << 2) ^ (animFrame & 0x03) ^ (!pet.alive);
  if (stateHash == lastAnimStateHash) return;
  lastAnimStateHash = stateHash;

  const int areaX = 36;
  const int areaY = GAME_Y + 34;
  const int areaW = 88;
  const int areaH = 54;
  const int cx = areaX + areaW / 2;
  const int cy = areaY + areaH / 2 + 2;
  ...
  if (pet.stage == 0) {
    ...
    tft.fillRoundRect(cx - r, cy - r + bob, r * 2, r * 2, 8, body);
    tft.drawRoundRect(cx - r, cy - r + bob, r * 2, r * 2, 8, COLOR_FG);

    // ears / tufts
    tft.drawLine(cx - r + 4, cy - r + 4 + bob, cx - r - 3, cy - r - 4 + bob, COLOR_FG);
    tft.drawLine(cx + r - 4, cy - r + 4 + bob, cx + r + 3, cy - r - 4 + bob, COLOR_FG);

    // eyes
    if (pet.sleeping) {
      tft.drawFastHLine(cx - 8, cy - 4 + bob, 5, COLOR_FG);
      tft.drawFastHLine(cx + 3, cy - 4 + bob, 5, COLOR_FG);
    ...
    // feet
    tft.drawFastHLine(cx - 10, cy + r - 1 + bob, 6, COLOR_FG);
    tft.drawFastHLine(cx + 4, cy + r - 1 + bob, 6, COLOR_FG);
  }
...
void drawHudAndBars() {
  ...
  const int bw = 26;
  drawBar(bx, by, bw, pet.hunger, moodColor(pet.hunger));
  drawBar(bx, by + 10, bw, pet.happiness, moodColor(pet.happiness));
  drawBar(bx, by + 20, bw, pet.energy, moodColor(pet.energy));
  drawBar(bx, by + 30, bw, pet.hygiene, moodColor(pet.hygiene));
  drawBar(130, by, bw, pet.health, moodColor(pet.health));
  drawBar(130, by + 10, bw, pet.discipline, moodColor(pet.discipline));
}
