...
constexpr uint32_t ANIM_TICK_MS = 350;
...
void drawPetSprite() {
  // The sprite is split into three redraw layers:
  // 1) base scene and body geometry
  // 2) face / animated overlay
  // 3) poop icons along the ground
  //
  // This is much faster than clearing and rebuilding the entire sprite box
  // every animation tick. Only the layer whose state changed gets touched.
  ...
  uint8_t bodyStage = pet.alive ? pet.stage : 7;
  uint8_t baseHash = (bodyStage << 1) | (pet.alive ? 1 : 0);
  uint8_t faceHash = (pet.sleeping << 5) ^ (pet.sick << 4) ^ ((pet.happiness / 20) << 1) ^ (animFrame & 0x01);

  bool redrawBase = (baseHash != lastSpriteBaseHash);
  bool redrawFace = redrawBase || (faceHash != lastSpriteFaceHash);
  bool redrawPoop = redrawBase || (pet.poop != lastSpritePoop);

  if (!redrawBase && !redrawFace && !redrawPoop) return;

  uint16_t body = COLOR_PINK;
  if (pet.stage == 0) body = COLOR_FG;
  else if (pet.stage == 2) body = COLOR_ACCENT;
  else if (pet.stage >= 4) body = rgb565(180, 255, 180);

  int bob = (animFrame & 1) ? 1 : 0;
  if (pet.sleeping) bob = 0;

  if (redrawBase) {
    lastSpriteBaseHash = baseHash;

    tft.fillRect(areaX, areaY, areaW, areaH, COLOR_BG);

    // Draw the static little scene first.
    tft.drawFastHLine(areaX, areaY + areaH - 8, areaW, COLOR_DIM);
    tft.fillRect(areaX, areaY + areaH - 7, areaW, 7, rgb565(0, 18, 0));

    if (!pet.alive) {
      // A simple memorial shell. The face is part of the base here because it
      // does not animate while the pet is down.
      tft.drawCircle(cx, cy - 4, 14, COLOR_DIM);
      tft.drawLine(cx - 6, cy - 8, cx - 2, cy - 4, COLOR_FG);
      tft.drawLine(cx - 2, cy - 8, cx - 6, cy - 4, COLOR_FG);
      tft.drawLine(cx + 2, cy - 8, cx + 6, cy - 4, COLOR_FG);
      tft.drawLine(cx + 6, cy - 8, cx + 2, cy - 4, COLOR_FG);
      tft.drawFastHLine(cx - 6, cy + 2, 12, COLOR_FG);
    } else if (pet.stage == 0) {
      tft.drawRoundRect(cx - 12, cy - 14, 24, 28, 8, COLOR_FG);
      tft.drawFastVLine(cx, cy - 10, 20, COLOR_DIM);
      tft.drawFastVLine(cx - 6, cy - 4, 8, COLOR_DIM);
      tft.drawFastVLine(cx + 6, cy - 4, 8, COLOR_DIM);
    } else {
      int r = (pet.stage <= 1) ? 12 : (pet.stage == 2 ? 14 : 16);
      tft.fillRoundRect(cx - r, cy - r, r * 2, r * 2, 8, body);
      tft.drawRoundRect(cx - r, cy - r, r * 2, r * 2, 8, COLOR_FG);

      // ears / tufts
      tft.drawLine(cx - r + 4, cy - r + 4, cx - r - 3, cy - r - 4, COLOR_FG);
      tft.drawLine(cx + r - 4, cy - r + 4, cx + r + 3, cy - r - 4, COLOR_FG);

      // feet
      tft.drawFastHLine(cx - 10, cy + r - 1, 6, COLOR_FG);
      tft.drawFastHLine(cx + 4, cy + r - 1, 6, COLOR_FG);
    }
  }

  if (pet.alive && redrawFace && pet.stage > 0) {
    lastSpriteFaceHash = faceHash;

    // Clear only the face band, not the whole body.
    const int faceX = cx - 14;
    const int faceY = cy - 12;
    const int faceW = 28;
    const int faceH = 24;
    tft.fillRect(faceX, faceY, faceW, faceH, body);

    // Re-draw border fragments that the face clear overlaps.
    int r = (pet.stage <= 1) ? 12 : (pet.stage == 2 ? 14 : 16);
    tft.drawRoundRect(cx - r, cy - r, r * 2, r * 2, 8, COLOR_FG);

    if (pet.sleeping) {
      tft.drawFastHLine(cx - 8, cy - 4, 5, COLOR_FG);
      tft.drawFastHLine(cx + 3, cy - 4, 5, COLOR_FG);
      tft.setCursor(cx + 18, cy - 18);
      tft.setTextColor(COLOR_ACCENT, body);
      tft.print("Z");
      if (animFrame & 1) {
      ...
      tft.drawLine(cx - 8, cy - 7 + bob, cx - 4, cy - 3 + bob, COLOR_FG);
      tft.drawLine(cx - 4, cy - 7 + bob, cx - 8, cy - 3 + bob, COLOR_FG);
      tft.drawLine(cx + 4, cy - 7 + bob, cx + 8, cy - 3 + bob, COLOR_FG);
      tft.drawLine(cx + 8, cy - 7 + bob, cx + 4, cy - 3 + bob, COLOR_FG);
    } else {
      tft.fillCircle(cx - 6, cy - 5 + bob, 2, COLOR_BG);
      tft.fillCircle(cx + 6, cy - 5 + bob, 2, COLOR_BG);
      if (animFrame & 1) {
        tft.drawPixel(cx - 6, cy - 8 + bob, COLOR_FG);
        tft.drawPixel(cx + 6, cy - 8 + bob, COLOR_FG);
      }
    }

    if (pet.happiness > 55) {
      tft.drawLine(cx - 4, cy + 3 + bob, cx - 1, cy + 6 + bob, COLOR_BG);
      tft.drawLine(cx + 1, cy + 6 + bob, cx + 4, cy + 3 + bob, COLOR_BG);
      tft.drawFastHLine(cx - 1, cy + 6 + bob, 2, COLOR_BG);
    } else if (pet.happiness > 25) {
      tft.drawFastHLine(cx - 3, cy + 5 + bob, 6, COLOR_BG);
    } else {
      tft.drawLine(cx - 4, cy + 7 + bob, cx - 1, cy + 4 + bob, COLOR_BG);
      tft.drawLine(cx + 1, cy + 4 + bob, cx + 4, cy + 7 + bob, COLOR_BG);
      tft.drawFastHLine(cx - 1, cy + 4 + bob, 2, COLOR_BG);
    }
  } else if (!pet.alive) {
    lastSpriteFaceHash = faceHash;
  }
