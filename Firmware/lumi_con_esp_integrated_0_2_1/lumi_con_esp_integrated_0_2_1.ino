/*
  ...
  ============================================================================
  Tutorial Edition / Lumia-Only Pet Control Revision
  ...
  1) Network/UI layer
     ...
     - Lets Lumia or tools push messages/status to a dedicated message page
  ...
  - Pet control is Lumia-only through /pet.
  - Pet change events include useful live telemetry values for Lumia.

  Boot key behavior
  ---------------------------------------------------------------------------
  - Hold Pico key 0 at boot -> reset Wi-Fi settings and restart
  - Hold Pico key 1 at boot -> disable pet mode until restart
...
constexpr uint8_t PET_DISABLE_BOOT_KEY = 1;
...
// Dedicated message page hold time.
// Edit this value if you want messages/status text to stay visible longer/shorter.
...
bool checkBootKeyHold(uint8_t watchKey);
...
void refreshPetDisabledMessagePage(bool resetHistory);
...
void clearUiMessages() {
  ...
  if (petModeEnabled) {
    ...
    refreshPetDisabledMessagePage(true);
  }
}
...
void refreshPetDisabledMessagePage(bool resetHistory) {
  if (resetHistory) {
    textPageLineCount = 0;
    appendTextPageMessage("PET MODE DISABLED");
    appendTextPageMessage("Until restart");
    appendTextPageMessage("Lumia msg/status only");
  }
...
void renderTextPage(bool force) {
  ...
  uint32_t token = (uint32_t)textPageLineCount ^ (textPageUntilMs << 1) ^ (uint32_t)textPageActive ^ (petModeEnabled ? 0x11 : 0x99);
  ...
  tft.setCursor(0, 0);
  tft.print(petModeEnabled ? "MESSAGES" : "PET MODE OFF");
...
void serviceTextPage(uint32_t now) {
  if (!petModeEnabled) {
    if (!textPageActive) {
      refreshPetDisabledMessagePage(false);
    } else {
...
bool checkBootKeyHold(uint8_t watchKey) {
  ...
  while (elapsedMs(millis(), start) < BOOT_HOLD_WINDOW_MS) {
    ...
    if (keyDown && elapsedMs(millis(), downAt) >= BOOT_HOLD_MS) return true;
    yield();
  }
  return false;
}
...
void handlePet() {
  ...
  if (!petModeEnabled) {
    return server.send(503, "application/json", "{\"ok\":false,\"error\":\"pet_mode_disabled_until_restart\",\"petModeEnabled\":false}");
  }
...
void setup() {
  ...
  setToast("Hold K0=WiFi reset", 1800);
  if (checkBootKeyHold(FACTORY_RESET_KEY)) {
  ...
  setToast("Hold K1=Pet OFF", 1800);
  if (checkBootKeyHold(PET_DISABLE_BOOT_KEY)) {
  ...
  if (petModeEnabled) {
    ...
    appendTextPageMessage(String("STATUS: ") + headerStatusBase);
    renderTextPage(true);
  }
}
...
void servicePicoEvents() {
  ...
  while (readPicoPacket(type, key)) {
    ...
    if (!petModeEnabled) {
      appendTextPageMessage(String("KEY: ") + String(key) + (isLongPress ? " long" : " short"));
      renderTextPage(true);
    }
  }
}
