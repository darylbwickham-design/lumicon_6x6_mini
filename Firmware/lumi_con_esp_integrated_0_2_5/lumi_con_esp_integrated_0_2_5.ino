...
constexpr uint32_t SPLASH_WELCOME_MS = 1100;
constexpr uint32_t SPLASH_BOOTOPTIONS_MS = 1200;
...
// Panel-corrected RGB565 values.
// This display swaps red and blue relative to the usual ST77XX constants.
#define RGB565_PANEL(r, g, b) (uint16_t)((((b) & 0xF8) << 8) | (((g) & 0xFC) << 3) | ((r) >> 3))

constexpr uint16_t COLOR_BG     = RGB565_PANEL(0, 0, 0);
constexpr uint16_t COLOR_FG     = RGB565_PANEL(255, 255, 255);
constexpr uint16_t COLOR_DIM    = 0x7BEF;
constexpr uint16_t COLOR_ACCENT = RGB565_PANEL(0, 255, 255);
constexpr uint16_t COLOR_WARN   = RGB565_PANEL(255, 255, 0);
...
bool postEventToPluginEx(uint8_t eventNumber, uint8_t keyIndex, const char* pressKind, uint32_t heldMs, uint32_t seq, const char* extraJson);
bool postEventToPlugin(uint8_t eventNumber, uint8_t keyIndex, const char* pressKind, uint32_t heldMs, uint32_t seq);
...
void drawCenteredText(int y, const String& text, uint16_t fg = COLOR_FG, uint16_t bg = COLOR_BG);
void waitSplash(uint32_t ms);
...
void addNoPetWelcomeLines() {
  if (uiMode == UI_MODE_CHAT) {
    appendTextPageMessage("Chat mode");
    appendTextPageMessage("Lumia messages only");
    appendTextPageMessage("Waiting for events...");
  } else if (uiMode == UI_MODE_NOPET_DEBUG) {
...
bool postEventToPlugin(uint8_t eventNumber, uint8_t keyIndex, const char* pressKind, uint32_t heldMs, uint32_t seq) {
  return postEventToPluginEx(eventNumber, keyIndex, pressKind, heldMs, seq, nullptr);
}
...
bool servicePetTransitionQueue(uint32_t now) {
  ...
  bool ok = postEventToPluginEx(petEventNumber(ev.code), 255, "pet", 0, seq, extra);
  lastPostOk = ok;
  if (!ok) return false;
...
// ============================================================================
// 11. BOOT SPLASH / PRESENTATION PAGES
// ============================================================================

void drawCenteredText(int y, const String& text, uint16_t fg, uint16_t bg) {
  int16_t x1, y1;
  uint16_t w, h;
...
void waitSplash(uint32_t ms) {
  uint32_t start = millis();
  while (elapsedMs(millis(), start) < ms) {
    yield();
  }
}
...
void drawLumiconLogoPage() {
  tft.fillScreen(COLOR_BG);
  tft.fillRoundRect(18, 18, 124, 34, 6, COLOR_ACCENT);
  drawCenteredText(30, "LUMI-CON", COLOR_BG, COLOR_ACCENT);
  tft.drawRoundRect(22, 66, 116, 24, 5, COLOR_DIM);
  drawCenteredText(74, "digital pet", COLOR_FG, COLOR_BG);
}

void drawMatrixMiniLogoPage() {
  tft.fillScreen(COLOR_BG);
  drawCenteredText(10, "6x6 MATRIX MINI", COLOR_ACCENT, COLOR_BG);

  const int cell = 8;
  const int gap = 2;
  const int gridW = (6 * cell) + (5 * gap);
  const int startX = (SCREEN_W - gridW) / 2;
  const int startY = 34;

  for (int row = 0; row < 6; ++row) {
    for (int col = 0; col < 6; ++col) {
      uint16_t c = COLOR_DIM;
      if (row == col || row + col == 5) c = COLOR_ACCENT;
      if ((row == 2 || row == 3) && (col == 2 || col == 3)) c = COLOR_GOOD;

      int x = startX + col * (cell + gap);
      int y = startY + row * (cell + gap);
      tft.fillRoundRect(x, y, cell, cell, 2, c);
    }
  }

  drawCenteredText(96, "matrix input ready", COLOR_FG, COLOR_BG);
}
...
void drawWelcomePage() {
  ...
  drawCenteredText(56, "digital pet + chat", COLOR_FG, COLOR_BG);
...
void drawBootOptionsPage() {
  ...
  tft.setCursor(8, 46);  tft.print("Hold K1  Chat mode");
  tft.setCursor(8, 62);  tft.print("Hold K2  noPet Debug");

  tft.drawFastHLine(8, 80, SCREEN_W - 16, COLOR_DIM);
  tft.setCursor(8, 92);
  tft.setTextColor(COLOR_DIM, COLOR_BG);
  tft.print("Waiting for boot key...");
}
...
void setup() {
  ...
  drawBootOptionsPage();
  waitSplash(SPLASH_BOOTOPTIONS_MS);
