...
module.exports = class LumiConBridgeIntegratedV113 extends Plugin {
  ...
  _getTimeoutMs() {
    const ms = Number(this.settings?.timeoutMs ?? 2000);
    return Number.isFinite(ms) && ms >= 250 ? Math.floor(ms) : 2000;
  }

  _getBaseUrl() {
    return normalizeBaseUrl(this.settings?.baseUrl);
  }

  _getStatusPath() {
    return normalizePath(this.settings?.statusPath ?? "/status");
  }
  ...
  async _setDeviceConnected(isConnected, options = {}) {
    ...
    if (!silent && !isConnected && failureToast) {
  ...
  async _silentHealthCheck() {
    if (this._offlineProbeInFlight) return;

    const baseUrl = this._getBaseUrl();
    if (!baseUrl) {
      await this._setDeviceConnected(false, { silent: false, failureToast: true });
      await this.lumia.setVariable("device_status_text", "OFFLINE");
      return;
    }
    ...
    try {
      const url = `${joinUrl(baseUrl, this._getStatusPath())}?silent=1&ts=${Date.now()}`;
      ...
      this._lastRssi = Number.isFinite(Number(parsed.device_rssi)) ? Number(parsed.device_rssi) : this._lastRssi;
      this._deviceId = typeof parsed.device_id === "string" ? parsed.device_id : this._deviceId;
      this._deviceIp = typeof parsed.device_ip === "string" ? parsed.device_ip : this._deviceIp;

      await this.lumia.setVariable("device_id", this._deviceId || "");
      await this.lumia.setVariable("device_ip", this._deviceIp || "");
      await this.lumia.setVariable("device_rssi", this._lastRssi || 0);
      await this.lumia.setVariable("seq", Number.isFinite(Number(parsed.seq)) ? Number(parsed.seq) : 0);
      await this.lumia.setVariable("device_last_seen", nowIso);
      ...
      await this._setDeviceConnected(false, { silent: false, failureToast: true });
      this._lastSeenMs = 0;
      await this._updateDeviceStatusText(Date.now(), true);
    } finally {
      this._offlineProbeInFlight = false;
    }
  }
  ...
  async _handleRequest(req, res) {
    try {
      ...
      if (deviceId && seq !== null) {
        ...
        if (seq < prev) {
          const gap = safeNowMs() - this._lastSeenMs;
          if (gap < SEQ_RESET_GAP_MS) {
            return this._sendJson(res, 200, ackPayload);
          }
      ...
      try {
        await this.lumia.log({ message: `[Lumi-Con v1.1.3] ${message}`, level: "error" });
      } catch {}
